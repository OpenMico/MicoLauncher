...
public class StringEscapeUtils {
    ...
    public static final CharSequenceTranslator ESCAPE_ECMASCRIPT = new AggregateTranslator(new LookupTranslator(new String[]{LrcRow.SINGLE_QUOTE, "\\'"}, new String[]{"\"", "\\\""}, new String[]{"\\", "\\\\"}, new String[]{"/", "\\/"}), new LookupTranslator(EntityArrays.JAVA_CTRL_CHARS_ESCAPE()), JavaUnicodeEscaper.outsideOf(32, 127));
    ...
    public static final CharSequenceTranslator UNESCAPE_JAVA = new AggregateTranslator(new OctalUnescaper(), new UnicodeUnescaper(), new LookupTranslator(EntityArrays.JAVA_CTRL_CHARS_UNESCAPE()), new LookupTranslator(new String[]{"\\\\", "\\"}, new String[]{"\\\"", "\""}, new String[]{"\\'", LrcRow.SINGLE_QUOTE}, new String[]{"\\", ""}));
        ...
        public int translate(CharSequence charSequence, int i, Writer writer) throws IOException {
            if (i == 0) {
                if (StringUtils.containsNone(charSequence.toString(), c)) {
                    ...
                    String str = b;
                    writer.write(StringUtils.replace(charSequence2, str, b + b));
                    writer.write(34);
                }
        ...
        public int translate(CharSequence charSequence, int i, Writer writer) throws IOException {
            if (i != 0) {
                ...
                if (StringUtils.containsAny(charSequence2, c)) {
                    writer.write(StringUtils.replace(charSequence2, b + b, b));
                } else {
                    writer.write(charSequence.toString());
                }
                return Character.codePointCount(charSequence, 0, charSequence.length());
