...
public abstract class BaseTrackSelection implements ExoTrackSelection {
    ...
    public BaseTrackSelection(TrackGroup trackGroup, int[] iArr, int i) {
        ...
        for (int i3 = 0; i3 < iArr.length; i3++) {
            this.b[i3] = trackGroup.getFormat(iArr[i3]);
        }
        Arrays.sort(this.b, $$Lambda$BaseTrackSelection$87DU9IFsf8BfbaTq134SJWyp0g.INSTANCE);
        this.tracks = new int[this.length];
        while (true) {
            int i4 = this.length;
            if (i2 < i4) {
                this.tracks[i2] = trackGroup.indexOf(this.b[i2]);
                i2++;
            } else {
                this.c = new long[i4];
                return;
    ...
    public int hashCode() {
        if (this.d == 0) {
            this.d = (System.identityHashCode(this.group) * 31) + Arrays.hashCode(this.tracks);
        }
        return this.d;
    }
