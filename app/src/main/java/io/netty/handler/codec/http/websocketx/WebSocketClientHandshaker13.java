...
public class WebSocketClientHandshaker13 extends WebSocketClientHandshaker {
    ...
    private static final InternalLogger a = InternalLoggerFactory.getInstance(WebSocketClientHandshaker13.class);
    private String b;
    ...
    protected FullHttpRequest newHandshakeRequest() {
        ...
        if (a.isDebugEnabled()) {
            a.debug("WebSocket version 13 client handshake key: {}, expected response: {}", c, this.b);
        }
        ...
        headers.add(HttpHeaderNames.UPGRADE, HttpHeaderValues.WEBSOCKET).add(HttpHeaderNames.CONNECTION, HttpHeaderValues.UPGRADE).add(HttpHeaderNames.SEC_WEBSOCKET_KEY, c).add(HttpHeaderNames.HOST, uri.getHost() + ':' + port);
        ...
        headers.add(HttpHeaderNames.SEC_WEBSOCKET_ORIGIN, sb2);
    ...
    protected WebSocketFrameDecoder newWebsocketDecoder() {
        return new WebSocket13FrameDecoder(false, this.c, maxFramePayloadLength(), this.e);
    }

    @Override // io.netty.handler.codec.http.websocketx.WebSocketClientHandshaker
    protected WebSocketFrameEncoder newWebSocketEncoder() {
        return new WebSocket13FrameEncoder(this.d);
    }
}
